...
Data Cleaning & Pre-processing — fixing duplicates, invalid values, inconsistent categories, and missing data across 24 columns, with every decision documented and justified (why mode vs. median vs. a placeholder category, why a column was dropped instead of imputed, etc.).
Exploratory Data Analysis — a storytelling walkthrough (Univariate → Bivariate → Multivariate) that answers specific business questions with charts, crosstabs, and written inferences, ending in a set of actionable recommendations.
...
<br>
🧹 Data Cleaning Steps
Removed exact duplicate rows across the full dataset.
Dropped comments, state, Timestamp — too sparse or irrelevant.
Age — coerced to numeric; unrealistic values (<18 or >100) set to NaN.
Gender — standardized free-text entries into Gender_clean (Male/Female/Other).
self_employed — missing values filled with "No".
treatment, family_history — checked for consistency (no missing values).
work_interfere — missing values filled with "Not applicable".
no_employees — fixed corrupted date-like entries, filled missing with mode.
Country, benefits, tech_company, remote_work — missing values filled with mode.
care_options, wellness_program, seek_help, anonymity — missing values filled with mode.
leave, mental_health_consequence, phys_health_consequence, coworkers, supervisor, mental_health_interview, phys_health_interview, mental_vs_physical, obs_consequence — missing values filled with mode (or "Don't know" for leave).
<br>
<br>
📊 EDA — Key Charts
Overall Treatment Need (Pie chart, univariate) — What share of employees need mental health treatment?
Employer Benefits vs Treatment-Seeking (Countplot, bivariate) — Does knowing about mental health benefits drive more treatment-seeking?
Family History vs Seeking Treatment (Grouped bar from crosstab, bivariate) — Does a family history of mental illness predict treatment-seeking?
Work Interference vs Treatment, by Gender (Countplot with hue, multivariate) — How does work interference vary by gender?
Correlation Heatmap (multivariate) — Relationships between age, treatment, family history, and workplace factors.
<br>
<br>
🔑 Key Findings
